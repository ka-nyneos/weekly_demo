...
function computeMetrics(data: FxItem[]): Metrics {
  ...
  data.forEach(i => {
    m.totalRequests++;
    if (["Sent", "Awaiting Bank Response", "Awaiting"].includes(i.status)) m.pendingBank++;
    ...
    if (i.amendmentStatus !== "No Amendment") {
      ...
      else if (i.amendmentStatus === "Amendment Confirmed") m.confirmedAmendments++;
    }
  });
    ...
    <Layout title="FX Forward Booking Confirmation Dashboard">
      ...
      <KpiSection
        ...
        items={[
          ...
          { label: "Total Booking Charges (YTD)", value: currencyFormatter.format(totalCharges) },
        ]}
      />
